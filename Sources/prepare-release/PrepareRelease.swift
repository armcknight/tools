...
struct PrepareRelease: AsyncParsableCommand {
    ...
    @Option(name: .long, help: "Directory containing the vrsn, migrate-changelog, and changetag binaries.")
    var toolsBin: String
    ...
    func run() async throws {
        let vrsn = "\(toolsBin)/vrsn"
        let migrateChangelog = "\(toolsBin)/migrate-changelog"
        let changetag = "\(toolsBin)/changetag"
        ...
        try await Shell.run(migrateChangelog, arguments: [changelog, newVersion, "--no-commit"])
